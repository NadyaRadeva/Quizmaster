...
void Administrator::approveQuiz(size_t quizId) {
    if (!quizManager) {
        std::cout << "Quiz manager is not set.\n";
        return;
    }
...
void Administrator::rejectQuiz(size_t quizId, const MyString& reason) {
    ...
    const Player* author = quiz->getQuizAuthor();
    if (author) {
...
void Administrator::sendMessage(const Player& player, const MyString& content) {
    Message msg(content, this, const_cast<Player*>(&player));

    Player& target = const_cast<Player&>(player);
